...
task :ping do
  begin
    ...
    XMLRPC::Client.new('rpc.pingomatic.com', '/').call('weblogUpdates.extendedPing', 'Vinod Kurup' , 'http://www.kurup.org/blog', 'http://www.kurup.org/blog/atom.xml')
  rescue LoadError
...
task :sync do
  ...
  puts `rsync -avz --delete "_site/" cartman:~/web/kurup.org/`
end
...
task :drafts do
  puts `find ./blog/_posts -type f -exec grep -H 'published: false' {} \\;`
end
